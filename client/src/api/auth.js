...
const fetchUserByUserId = async (token, id) => {
  try {
    const { data } = await axios.get(`http://192.168.1.65:8000/api/users/highlight/${id}`, {
      headers: { Authorization: `Bearer ${token}` },
    })
    return data
  } catch (e) {
...
const setNotificationTrue = async (token, { userId, notificationId }) => {
  try {
    const { data } = await axios.patch(`http://192.168.1.65:8000/api/users/${userId}/notification/${notificationId}`, {
      headers: { Authorization: `Bearer ${token}` },
    })
    return data
  } catch (e) {
